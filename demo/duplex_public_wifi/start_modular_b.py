#!/usr/bin/env python3
"""
RP5-B Modular Full-Duplex Starter (Public WiFi)
================================================
...
Usage:
    python demo/duplex_public_wifi/start_modular_b.py
    python demo/duplex_public_wifi/start_modular_b.py --peer-ip 10.249.55.123
...
def resolve_mdns_hostname(hostname):
    """
    Resolve mDNS hostname (e.g., 'test1-desktop.local') to IP address
...
def detect_peer_ip_auto():
    ...
    # Extract network prefix (e.g., 10.249.98.x -> 10.249.98)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RP5-B Full-Duplex Starter (Public WiFi)')
    parser.add_argument('--peer-ip', type=str, help='Peer IP address (overrides config)')
    parser.add_argument('--config', type=str, default='configs/rp5b_public.yaml', help='Config file path')
    ...
    print(f"{Colors.BLUE}{'='*60}{Colors.NC}")
    print(f"{Colors.BLUE}🚀 RP5-B Modular Full-Duplex Starter (Public WiFi){Colors.NC}")
    print(f"{Colors.BLUE}{'='*60}{Colors.NC}")
    print()
